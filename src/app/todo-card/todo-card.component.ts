...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, GameComponent, MatCardModule],
  templateUrl: './todo-card.component.html',
  styleUrl: './todo-card.component.scss',
})
export class TodoCardComponent {
  // this is our array where we display our random text for every card
  cardAction = [
    {
      title: 'Waterfall',
      description:
  ...
  // input from current played card
  @Input() card: string = '';
  title = '';
  description = '';
  ...
  // to split the image number from string to number
  ngOnChanges() {
    if (this.card) {
